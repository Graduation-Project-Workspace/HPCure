...
object DicomUtils {
    ...
    private const val MAX_PIXEL_VALUE = 65535 // For 16-bit images
    ...
    fun convertDicomToBitmap(dicomFile: File): Bitmap {
        try{
            ...
            val dis = DicomInputStream(dicomFile).use { dis ->
                val attrs = dis.readDataset(-1, -1)

                // Check if required attributes exist
                if (!attrs.contains(org.dcm4che3.data.Tag.Columns) ||
                    !attrs.contains(org.dcm4che3.data.Tag.Rows) ||
                    !attrs.contains(org.dcm4che3.data.Tag.PixelData)) {
                    throw IOException("Missing required DICOM attributes")
                }
            ...
            Log.e("DicomUtils", "Error converting DICOM to bitmap", e)
            throw e
        }
